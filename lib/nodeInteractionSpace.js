...
function NodeInteractionSpace(targetFolder, returnFolder) {
    var swarmHandlersSubscribers = {};
    var queueHandler = null;
    var responseQueue = null;

    var queue = folderMQ.createQue(folder, (err, result) => {
        if(err){
           throw err;
        }
    });
    ...
    return swarmInteract.newInteractionSpace((role == NodeInteractionSpace.prototype.PRODUCER) ? producerCOMM : consumerCOMM);
}

var spaces = {};

module.exports.createConsumerInteractionSpace = function (targetFolder, returnFolder) {
    var index = targetFolder+returnFolder;
    if(!spaces[index]){
        spaces[index] = new NodeInteractionSpace(targetFolder, returnFolder);
    }else{
